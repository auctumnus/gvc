...
const days = computed(() =>
  eachDayOfInterval({ start: startOfCon.value, end: endOfCon.value }),
);

type CalendarTimeWithKind = CalendarTime & {
...
const eventsWithConcurrency: ComputedRef<EventT[][]> = computed(() => {
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    for (let j = 0; j < dayEvents.length; j++) {
      ...
      dayEventsWithConcurrency.push({
        ...event,
        concurrentWith: concurrentWith.length,
      });
    }
...
const eventsPerDay = computed(() =>
  eventsWithConcurrency.value.map((events, i) => ({
    day: days.value[i],
    events,
  })),
);
...
<template>
  <div class="calendar">
    ...
    <div class="calendar-controls">
      ...
      <button @click="currentWeek++" :disabled="currentWeek === lastWeek">
        Next
      </button>
    </div>
