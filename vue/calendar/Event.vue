...
const hoursFromTop = computed(() => {
  ...
  const difference = differenceInMinutes(
    start,
    startOfDay,
  ); /* can't do differenceInHours bc it returns an integer */
...
<template>
  <div :class="classes" :style="styles" ref="el" @click="focus">
    ...
    <h3 v-if="!event.startedYesterday && event.game_name">
      {{ event.game_name }}
    </h3>
    <span
      v-if="!event.startedYesterday && event.entries_count !== undefined"
      :aria-label="accessibleLabel"
      :class="underMin ? 'under-min-count' : ''"
      >{{ readableLabel }}</span
    >
    <event-modal
      v-if="event.kind !== 'current'"
      :visible="eventModalVisible"
      :event="event"
      @close="eventModalVisible = false"
    />
  </div>
</template>
