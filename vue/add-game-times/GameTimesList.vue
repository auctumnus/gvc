...
<template>
  <p>All times on this page are in your browser's time zone ({{ Intl.DateTimeFormat().resolvedOptions().timeZone }}).</p>
  ...
  <p v-if="data.minPlayers > 0">
    ...
    <strong>under the minimum player count</strong> for this game <span class="nowrap">(currently set
    to <strong>{{ data.minPlayers }}</strong
    >).</span>
  </p>
  </div>
