...
/* TODO: this is just copied from add time modal. it should probably be made more DRY */
...
import { addMinutes, type Interval } from "date-fns";
...
const times = computed(() => {
  ...
  const halfHours = eachMinuteOfInterval(interval).filter(
    (d) => d.getMinutes() % 30 === 0,
  );
...
const day = ref(startOfDay(new Date(props.gameTime.start)));

const time = ref(new Date(props.gameTime.start));

const duration = ref(
  (props.gameTime.end - props.gameTime.start) / 1000 / 60 / 60,
);

const isPastEnd = computed(() => {
  return (
    compareAsc(
      addHours(time.value, duration.value),
      addMinutes(endOfCon, 1),
    ) === 1
  );
});
...
const save = async () => {
  ...
  await data.editTime(props.gameTime.id, start.valueOf(), end.valueOf());
  emit("close");
